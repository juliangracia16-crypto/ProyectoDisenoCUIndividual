...
public class ClientesBO implements IClientesBO {
    ...
    public ClienteLogueadoDTO registrarCliente(NuevoClienteDTO clienteDTO) throws NegocioException{
        Cliente cliente = DtosAEntidadesYViceversaAdapter.adaptarClienteDTO(clienteDTO);      
        try {
            Cliente clienteGuardado = fachada.registrarCliente(cliente);
            ClienteLogueadoDTO clienteAgregadoDTO = DtosAEntidadesYViceversaAdapter.adaptarClienteEntidad(clienteGuardado);
    ...
    public ClienteLogueadoDTO buscarClientePorId(String id) throws NegocioException{
        try {
            ...
            ClienteLogueadoDTO clienteDTO = DtosAEntidadesYViceversaAdapter.adaptarClienteEntidad(cliente);
            return clienteDTO;
        } catch (PersistenciaException ex) {
            throw new NegocioException("Error al registrar el cliente",ex);
        }
    ...
    public List<ClienteLogueadoDTO> consultarClientes() throws NegocioException{
        try {
            ...
            for(Cliente c: clientes){
                ClienteLogueadoDTO cliente = DtosAEntidadesYViceversaAdapter.adaptarClienteEntidad(c);
